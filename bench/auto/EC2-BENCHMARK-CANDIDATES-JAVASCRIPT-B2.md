...
optimizations on EC2, **head-to-head against the pinned `bench-baseline-javascript-b2` baseline**, using
...
> **Three candidates also carry a benchmark-inert harness/lint edit — this does NOT void the
> apples-to-apples comparison.** The funnel repairs a pre-existing lint failure in the profiling
> harness so its `mvn` gate can run, and that repair leaks into the branch diff. Specifically:
> `anyserializer-header-read-combine` edits `eslint.config.mjs`; `array-preallocate-and-numeric-bulk-loop`
> edits `eslint.config.mjs` **and** `lib/driver/util/profiling-application.mjs`;
> `streaming-loop-bulk-count-tighten` edits `lib/driver/util/profiling-application.mjs`. The
> `eslint.config.mjs` changes are lint-config only (zero runtime effect). The
> `profiling-application.mjs` changes are prettier/lint reformatting; the only *functional* delta
> (in `array-preallocate…`) is confined to `runThroughputTest`, whereas the candidate eval runs the
> **latency** path (`runLatencyTest`), which is byte-for-byte identical in behavior on every arm. So
> these files change no measured code path. The **"Files changed" column below counts only the
> real deser change** — treat the harness/lint file as noise the merge should drop, not as part of
> the optimization.
...
commits land — so the baseline is pinned to the immutable fork-point SHA **`4d36385d22`**, never the
branch tip. A moving tip would silently mix extra commits into the comparison.

> **This is the SECOND JS candidate batch — a different fork point and a separate namespace from the
> first.** The first (2026-07) deep JS run forked from `5d575e51f8`, used the baseline branch
> `bench-baseline-javascript`, and its 14 arms' ledgers live under `s3://kirill-tp-benchmarks/cand-results/javascript/`
> (that is the record of the earlier +75% `sync-decode-core-buffered` result — **do not overwrite it**).
> This batch forks from **`4d36385d22`** ("Make report phase read-only"), pins its baseline to the
> new branch **`bench-baseline-javascript-b2`**, and writes results to the fresh prefix
> `s3://kirill-tp-benchmarks/cand-results/javascript-2026-08/`, so the two runs never mix. The first
> run's branches and ledgers are left untouched.

For this run the baseline branch **`bench-baseline-javascript-b2`** = `4d36385d22` exactly — **no extra
...
- **Baseline arm** = `bench-baseline-javascript-b2` (= `4d36385d22`).
- **Each candidate arm** = `bench-baseline-javascript-b2` + exactly one deser commit (the
...
> ```bash
> git branch bench-baseline-javascript-b2 4d36385d22    # immutable fork point
> ```

### The candidate set (6 branches — the verified-green funnel set)

The current set is **6 gremlin-javascript candidates**, each a small commit on `4d36385d22`, all
full-suite green (`mvn clean install` — mocha unit + graphbinary integration + cucumber-js features
+ node examples, run under the docker `glv-js` profile) — produced by the run of
`glv-correctness-funnel.workflow.js` (`args: { glv: "javascript" }`). The funnel produces this set
**fresh each run — it is non-deterministic, do not assume a fixed set.**

The table is ordered smallest blast radius first. "Files" counts only the **real deser** files each
branch changes vs `4d36385d22`; the benchmark-inert `eslint.config.mjs` / `profiling-application.mjs`
harness edits on three arms (see the note above §Baseline) are excluded from the count.

| Branch (`auto/cand-javascript-…`) | Files changed | Blast radius | Optimization |
|---|---|---|---|
| `long-safe-int-bigint-avoidance` | `LongSerializer.js` | 1 file, safe | avoid a BigInt allocation for in-range Long values |
| `streamreader-inline-fast-path` | `StreamReader.js` | 1 file, safe | inline a synchronous fast path into the `StreamReader` read methods |
| `streaming-loop-bulk-count-tighten` | `GraphBinaryReader.js` (+ harness `profiling-application.mjs`) | 1 file, safe | single-read bulk-count parsing in the streaming loop of `GraphBinaryReader` |
| `array-preallocate-and-numeric-bulk-loop` | `ArraySerializer.js` (+ harness `eslint.config.mjs`, `profiling-application.mjs`) | 1 file, safe | preallocate the `ArraySerializer` result array + `Number` (not BigInt) bulk loop counter |
| `anyserializer-header-read-combine` | `AnySerializer.js`, `StreamReader.js` (+ harness `eslint.config.mjs`) | 2 files, medium | combine the `AnySerializer` type-code + value-flag reads into one buffer read |
| `scalar-serializer-async-passthrough-removal` | `ByteSerializer.js`, `DoubleSerializer.js`, `FloatSerializer.js`, `IntSerializer.js`, `ShortSerializer.js` | 5 files, medium | drop the async wrapper on the fixed-width scalar deserializers |

> **No wide-blast-radius arm this batch.** Unlike the first JS run (whose `sync-decode-core-buffered`
> touched 29 files), the largest arm here is `scalar-serializer-async-passthrough-removal` at 5
> one-line-family serializer edits. Every arm is a small, self-contained change; none needs its own
> merge bucket.

All 6 arms + the baseline `bench-baseline-javascript-b2` are pushed to the fork
`git@github.com:kirill-stepanishin/tinkerpop.git`.
...
git branch bench-baseline-javascript-b2 4d36385d22 2>/dev/null || true
# baseline first, then the 6 candidate arms (auto/cand-javascript-<id>):
for b in auto/cand-javascript-long-safe-int-bigint-avoidance \
         auto/cand-javascript-streamreader-inline-fast-path \
         auto/cand-javascript-streaming-loop-bulk-count-tighten \
         auto/cand-javascript-array-preallocate-and-numeric-bulk-loop \
         auto/cand-javascript-anyserializer-header-read-combine \
         auto/cand-javascript-scalar-serializer-async-passthrough-removal ; do
  git branch --track "$b" "origin/$b" 2>/dev/null || true   # provisioned box: "fork/$b"
done
git checkout bench-baseline-javascript-b2
git log --oneline -1           # confirm baseline = 4d36385d22
...
bench run --glv javascript --test protocol-overhead --size tiny \
  ...
  --output-dir ~/cand-results/javascript-2026-08/_connectivity
cat ~/cand-results/javascript-2026-08/_connectivity/ledger.csv
```
...
RESULTS=~/cand-results/javascript-2026-08     # namespaced by GLV so a JS run never collides with python/go/dotnet
mkdir -p "$RESULTS"

# branch list: baseline MUST be first, then the 6 green candidates
BRANCHES=(
  bench-baseline-javascript-b2
  auto/cand-javascript-long-safe-int-bigint-avoidance
  auto/cand-javascript-streamreader-inline-fast-path
  auto/cand-javascript-streaming-loop-bulk-count-tighten
  auto/cand-javascript-array-preallocate-and-numeric-bulk-loop
  auto/cand-javascript-anyserializer-header-read-combine
  auto/cand-javascript-scalar-serializer-async-passthrough-removal
)
```
...
run_arm () {
  ...
  local tag="${branch##*/}"                 # e.g. cand-javascript-anyserializer-header-read-combine or bench-baseline-javascript-b2
...
- **Latency comes from the UNPROFILED medium pass; never from a profiled cell.** Always run one arm
  first (`run_arm bench-baseline-javascript-b2 1`) and confirm the unprofiled medium row lands before
...
- The explicit `npm run build` after each checkout is the load-bearing JS difference from the python
  ...
  benchmarking. Every arm in this batch is a small change (≤5 files), so a build error is unlikely,
  but the rebuild is still mandatory — the profiling app loads the compiled `build/`, not `lib/`.
...
git -C "$REPO" checkout bench-baseline-javascript-b2   # leave the tree on baseline when done
```

> **Time budget.** Unprofiled medium ≈ a couple min/cell; the full **3-sweep × 7-arm** medium+tiny
> loop (baseline + 6 candidates) is ~1–1.5 hours. Run under `tmux`/`screen`, kick it off, detach
...
The reproducible path is **`bench/auto/candidate-analysis-javascript-b2.ipynb`** (pandas + plotly): it
...
```bash
aws s3 sync "$RESULTS" "s3://kirill-tp-benchmarks/cand-results/javascript-2026-08/" \
  --exclude "*.cpuprofile"      # V8 profiles are large + not needed by the notebook's ledger logic
```

Then open `candidate-analysis-javascript-b2.ipynb` anywhere with AWS creds for the bucket. Its
parameters cell is already set for this run:

```python
GLV                  = 'javascript'
BUCKET               = 'kirill-tp-benchmarks'
PREFIX               = f'cand-results/{GLV}-2026-08'     # -> cand-results/javascript-2026-08 (2nd JS batch)
BASELINE             = 'bench-baseline-javascript-b2'     # baseline arm's dir tag (= run_arm's ${branch##*/})
...
> `${branch##*/}`, so the baseline dir is `bench-baseline-javascript-b2`. This is the dotnet convention
...
`bench-baseline-javascript-b2` (subject to the §5c validity gates).
...
```
GLV: javascript   Baseline branch (immutable fork point): bench-baseline-javascript-b2 = 4d36385d22
...
|-------------------------------------------------------|----------------------------|--------------|----------------------------------|--------------|--------------|
| bench-baseline-javascript-b2                          | (reference)                |              | —                                | <N>          | —            |
| cand-javascript-long-safe-int-bigint-avoidance        |                            |              |                                  | (must = N)   | 1 file       |
| cand-javascript-streamreader-inline-fast-path         |                            |              |                                  | (must = N)   | 1 file       |
| cand-javascript-streaming-loop-bulk-count-tighten     |                            |              |                                  | (must = N)   | 1 file       |
| cand-javascript-array-preallocate-and-numeric-bulk-loop |                          |              |                                  | (must = N)   | 1 file       |
| cand-javascript-anyserializer-header-read-combine     |                            |              |                                  | (must = N)   | 2 files      |
| cand-javascript-scalar-serializer-async-passthrough-removal |                      |              |                                  | (must = N)   | 5 files      |
```

- **Win** = **unprofiled medium wall-clock median improves** vs `bench-baseline-javascript-b2` (the
...
- Each candidate is a clean small commit on `bench-baseline-javascript-b2` — a winner merges with
  `git merge --no-ff auto/cand-javascript-<id>`. Merge **one at a time** and re-bench; two arms
  touch `StreamReader.js` so they may be **substitutes, not additive**:
  - `StreamReader.js`: `streamreader-inline-fast-path`, `anyserializer-header-read-combine`
  - `AnySerializer.js`: `anyserializer-header-read-combine`
  - `GraphBinaryReader.js`: `streaming-loop-bulk-count-tighten`
  - `ArraySerializer.js`: `array-preallocate-and-numeric-bulk-loop`
  - `LongSerializer.js`: `long-safe-int-bigint-avoidance`
  - fixed-width scalars (`Byte`/`Double`/`Float`/`Int`/`ShortSerializer`): `scalar-serializer-async-passthrough-removal`

  The other four arms touch disjoint files, so they are candidates to stack additively — but still
  re-bench after each merge to confirm.
- **When you merge a winner, drop the harness/lint churn.** Three arms carry benchmark-inert
  `eslint.config.mjs` / `profiling-application.mjs` edits (see the note under §Baseline). Those are
  not part of the optimization — cherry-pick only the deser file(s), or revert the harness file out of
  the merge, so the committed change is the real diff and nothing more.
...
```bash
git -C ~/tinkerpop-4 checkout bench-baseline-javascript-b2   # tree back on baseline
...
# read
# primary (decides winners): unprofiled medium wall-clock median vs bench-baseline-javascript-b2 (§5a)
...
- `bench/auto/candidate-analysis-javascript-b2.ipynb` — the reproducible pandas/plotly notebook that reads
  the S3-published results and renders the latency delta, guards, charts, and PASS/FAIL verdict (§5·0).
  Set `GLV='javascript'`, `BASELINE='bench-baseline-javascript-b2'`, `ARM_INCLUDE_PREFIXES=('cand-javascript-',)`.
