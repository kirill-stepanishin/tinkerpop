...
CRITICAL — STAY ALIVE, NEVER STALL (a single MEDIUM invocation runs ~13min, far past the 3-min
no-progress watchdog AND the 10-min Bash cap, so you CANNOT run it in the foreground):
  - Launch EACH bench invocation with the Bash tool's run_in_background:true (one invocation at a
    time — the gate is strictly serial; only one arm may touch the server at once).
  - Then POLL in a loop with SHORT, returning Bash calls (e.g. tail the run's log / ls the
    --output-dir for ledger.csv, sleep ~60s, repeat). Every poll call MUST return in well under 3
    minutes so the watchdog always sees fresh progress. Emit a one-line progress note each poll.
  - An invocation is DONE when its ledger.csv row appears (status=ok) or the bg task exits. Only
    then launch the next invocation (next size, then next arm, then next round).
  - FORBIDDEN: blocking/foreground bench calls; chaining all rounds into one long script; nohup /
    disown / setsid / 'cmd & exit' or ANY detached process that can outlive you — a prior run leaked
    an orphaned looping driver this way. If you background it, you MUST be the one polling and
    reaping it; never hand it off to launchd.

Collect per-round medians (the 'median' column of each ledger.csv row) into
candidateMediumWallReps / baselineMediumWallReps / candidate/baseline TinyWallReps (${BENCH_ROUNDS}
numbers each).
