...
# GLV Benchmark Harness (3.7 branch)
...
- **Per-branch.** All branch-specific settings live in this directory's
  `config.yaml` and `matrix.yaml`. This is the **3.7** copy: transport `ws`
  (WebSockets), `concurrency_model: multiplex` (concurrency realized via WS
  multiplexing knobs, not a connection pool of size C — see below).
- **Version-ignorant.** The orchestrator code never reasons about "the other
  version". The 4.0 branch carries its own `config.yaml`/`matrix.yaml` with
  `transport: http` and `concurrency_model: pool`. The same Python code runs on
...
### 3.7 concurrency model: WS multiplexing

On this branch concurrency `C` is **not** a connection pool of size `C`.
Instead, each GLV realizes `C` in-flight requests over WebSockets differently
(`concurrency_model: multiplex`, lifted verbatim from the old
`cat4-scaling-curve.sh`):

- **java / dotnet** multiplex over `pool × maxInProcessPerConnection ≈ C`, with
  `maxInProcess` capped per GLV (java 64, dotnet 16) and the pool sized to cover
  the remainder (`pool = ceil(C / cap)`; for `C ≤ cap`, `pool = 1`). For java
  `C=256` → `pool=4, maxInProcess=64`. Java pins submission parallelism at 16
  (connections, not threads, drive concurrency over WS); dotnet drives
  `parallelism = C`.
- **javascript** multiplexes via a small fixed pool (size 8) driven by
  `parallelism = C`, with no explicit in-process knob.
- **go / python** do not multiplex (`pool = C`); their pool-model argv already
  matches the old `cat4`, so no multiplex branch applies.
...
`config.py` resolves the version-stamped java path automatically (the 3.7
standalone dir is `apache-tinkerpop-gremlin-driver-3.7.*-SNAPSHOT-standalone`).
Run `bench run --dry-run` to confirm every launcher resolves before a real run;
any unbuilt GLV shows as `MISSING`.

> **3.7 path note:** on this branch the JavaScript launcher lives **directly**
> under `gremlin-javascript/...` (no `gremlin-js/` parent). On 4.0 it lives
> under `gremlin-js/gremlin-javascript/...` — this is the key layout
> difference between the two branches.

> **3.7 transport note:** because `transport: ws`, the adapter appends
> `--transport ws` to the **python** launcher (matching the old `common.sh`
> `run_python` wrapper). On 4.0 (`http`) that flag is omitted.

You will also need a **running Gremlin Server with the WebSocket channelizer**
reachable from the client (see
...
# Verify the multiplexed java argv (pool=4, maxInProcess=64) for C=256.
bench run --dry-run --test scaling-curve --glv java --concurrency 256

# Single GLV, single test, one query size, against a real WS server (EC2).
...
bench run --glv java --test scaling-curve --concurrency 256 \
...
.NET) that would be launched. On 3.7 this is the fastest way to confirm the WS
**multiplex** arithmetic (e.g. `minConnectionPoolSize`/`maxInProcessPerConnection`
on java/dotnet) before committing to a run. It needs **no server and no built
GLVs**.
...
| `peak-throughput`   | throughput | Absolute throughput ceiling, **java only**, WS multiplexed (3.7 pins `pool=128` since each connection carries up to `maxInProcess=64`). Other GLVs peak inside the scaling curve. | single point |
...
On this 3.7 branch (`concurrency_model: multiplex`), scaling-curve concurrency
`C` is realized over WebSockets via the multiplex knobs described
[above](#37-concurrency-model-ws-multiplexing) — `pool × maxInProcessPerConnection`
for java/dotnet, a small fixed pool for javascript, and a plain `pool = C` for
go/python.
...
resolve, the WS server answers, each GLV emits a parseable `RESULT_JSON:` line,
the multiplex argv actually executes, stats compute, and a row appends. It is
**not** a valid measurement because:
