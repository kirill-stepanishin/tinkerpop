...
# Fix parallelism, sweep pool size.
# HTTP transport: pool directly limits concurrency, so expect steep curves.
...
for POOL in 64 128 256 512 1000 2000 5000; do
  echo "--- Java pool=$POOL ---"
  run_java \
    testType 1 \
    host "$BENCH_HOST" \
...
for POOL in 8 16 32 64 128 256 512; do
...
for POOL in 32 64 128 256 512 1024 5000; do
  echo "--- Go pool=$POOL ---"
  run_go \
    --test-type throughput \
    --host "$BENCH_HOST" \
...
for POOL in 8 16 32 64 128 256; do
  echo "--- JS pool=$POOL ---"
  run_js \
    --test-type throughput \
    --host "$BENCH_HOST" \
...
for POOL in 64 128 256 512 1024 2000; do
  echo "--- .NET pool=$POOL ---"
  run_dotnet \
    --test-type throughput \
    --host "$BENCH_HOST" \
