...
export default class FloatSerializer {
  ...
  async deserialize(reader) {
    ...
    if (type_code !== this.ioc.DataType.FLOAT) {
      throw new Error(`FloatSerializer: unexpected {type_code}=0x${type_code.toString(16)}`);
    }
    ...
    if (value_flag !== 0x00) {
      throw new Error(`FloatSerializer: unexpected {value_flag}=0x${value_flag.toString(16)}`);
    }
