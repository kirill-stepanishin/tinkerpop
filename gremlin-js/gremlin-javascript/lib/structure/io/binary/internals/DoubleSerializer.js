...
export default class DoubleSerializer {
  ...
  async deserialize(reader) {
    ...
    if (type_code !== this.ioc.DataType.DOUBLE) {
      throw new Error(`DoubleSerializer: unexpected {type_code}=0x${type_code.toString(16)}`);
    }
    ...
    if (value_flag !== 0x00) {
      throw new Error(`DoubleSerializer: unexpected {value_flag}=0x${value_flag.toString(16)}`);
    }
