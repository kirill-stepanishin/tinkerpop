...
async Task RunLatencyTest(Config cfg)
{
    ...
    using var client = new GremlinClient(server, connectionPoolSettings: poolSettings);
    ...
    GremlinClient client, Config cfg, int requestCount, Random random)
{
    ...
    for (int i = 0; i < requestCount; i++)
    {
        ...
        tasks[i] = Task.Run(async () =>
        {
            ...
            try
            {
                var results = await client.SubmitAsync<dynamic>(script);
                _ = results.Count;
...
async Task InitializeGraph(Config cfg)
{
    ...
    var r1 = await client.SubmitAsync<dynamic>("graph.clear()");
    _ = r1.Count;
    Console.WriteLine("Cleared existing 'graph'");

    var r2 = await client.SubmitAsync<dynamic>("TinkerFactory.generateModern(graph)");
    _ = r2.Count;
    Console.WriteLine("Modern graph loaded");
}
...
Config ParseArgs(string[] args)
{
    ...
    for (int i = 0; i < args.Length; i++)
    {
        ...
        switch (key)
        {
            ...
            case "pool-size":
            case "poolSize":
                cfg.PoolSize = int.Parse(args[++i]); break;
            case "max-in-process":
            case "maxInProcessPerConnection":
                cfg.MaxInProcessPerConnection = int.Parse(args[++i]); break;
...
class Config
{
    ...
    public string Script { get; set; } = "1+1";
    ...
    public int MaxInProcessPerConnection { get; set; } = 64;
